...
class TestCreateOrder:
    ...
    def test_create_order_without_ingredients(self):
        response = requests.post(f'{URLS.MAIN_URL}{Handlers.CREATE_ORDER}')
        assert response.status_code == 400 and response.json()['message'] == TextConstants.missing_ingredients_error
    ...
    def test_create_order_with_invalid_hash_ingredients(self, create_and_delete_user):
        ...
        assert response.status_code == 500 and TextConstants.internal_server_error in response.text